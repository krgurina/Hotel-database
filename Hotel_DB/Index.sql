...
CREATE INDEX service_employee_id_index ON SERVICE_TYPES(service_type_employee_id);
...
CREATE INDEX photo_room_type_index ON PHOTO(photo_room_type_id);

CREATE INDEX guest_name_surname_index ON GUESTS(guest_name, guest_surname);
CREATE INDEX guest_email_index ON GUESTS(guest_email);

CREATE INDEX employee_name_surname_index ON EMPLOYEES(employee_name, employee_surname);
CREATE INDEX employee_position_index ON EMPLOYEES(employee_position);
...
CREATE INDEX booking_guest_index ON BOOKING(booking_guest_id);
