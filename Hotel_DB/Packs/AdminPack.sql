...
PROCEDURE UpdateEmployee(
    ...
    PROCEDURE CHECK_OUT_GUESTS;
    PROCEDURE CHECK_IN_GUESTS;
...
BEGIN
    ...
    END IF;
    if p_employee_hire_date is not NULL then
    ...
    if p_employee_email is not NULL then
        IF REGEXP_LIKE(p_employee_email, '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,4}$') = FALSE THEN
            RAISE_APPLICATION_ERROR(-20005, 'Неправильный формат email.');
        END IF;
    end if;
...
BEGIN
    ...
    IF p_room_type_daily_price <= 0 THEN
        RAISE_APPLICATION_ERROR(-20007, 'Ежедневная стоимость типа комнаты должна быть больше 0.');
    END IF;
    ...
    IF existing_count > 0 THEN
        RAISE_APPLICATION_ERROR(-20002, 'Тип комнаты с таким именем уже существует.');
    END IF;
...
BEGIN
    ...
    IF p_new_room_type_capacity IS NOT NULL THEN
        IF p_new_room_type_capacity < 0 THEN
            RAISE_APPLICATION_ERROR(-20006, 'Вместимость типа комнаты должна быть больше 0.');
        END IF;
    END IF;

    IF p_new_room_type_daily_price IS NOT NULL THEN
...
BEGIN
    IF p_tariff_type_daily_price <= 0 THEN
        RAISE_APPLICATION_ERROR(-20008, 'Ежедневная цена тарифа должна быть больше 0.');
    END IF;
  ...
  -- Проверка наличия такого же тарифа
    ...
    IF v_existing_count > 0 THEN
        RAISE_APPLICATION_ERROR(-20002, 'Тариф с таким именем уже существует.');
    END IF;
...
BEGIN
    ...
    IF v_existing_tariff_type.TARIFF_TYPE_ID IS NULL THEN
        RAISE_APPLICATION_ERROR(-20002, 'Тариф с указанным ID не существует.');
    END IF;

    IF p_tariff_type_daily_price IS NOT NULL THEN
        IF p_tariff_type_daily_price <= 0 THEN
            RAISE_APPLICATION_ERROR(-20008, 'Новая ежедневная цена тарифа должна быть больше 0.');
        END IF;
    END IF;
...
BEGIN
    ...
    IF NOT REGEXP_LIKE(p_room_number, '^[1-9][0-9]{2}[а-б]?$') THEN
        RAISE_APPLICATION_ERROR(-20009, 'Некорректный формат номера комнаты.');
    END IF;
    ...
    IF v_room_count > 0 THEN
        RAISE_APPLICATION_ERROR(-20002, 'Комната с таким номером уже существует.');
    END IF;
...
BEGIN
    ...
    IF v_booking_count = 0 THEN
        RAISE_APPLICATION_ERROR(-20010, 'Гость не имеет брони на указанный период.');
    END IF;

    IF p_service_start_date >= p_service_end_date THEN
        RAISE_APPLICATION_ERROR(-20011, 'Дата начала услуги должна быть меньше даты окончания.');
    END IF;
...
BEGIN
  ...
  LOOP
    ...
    v_cost := ADMIN.USERPACK.CALCULATE_STAY_COST(v_booking_id);
    IF v_cost IS NOT NULL THEN
      DBMS_OUTPUT.PUT_LINE('За проживание в отеле с вас ' || TO_CHAR(v_cost, '9999.99') || 'р.');
    ELSE
      RAISE_APPLICATION_ERROR(-20016,'Не удалось рассчитать стоимость проживания.');
    END IF;
...
PROCEDURE CHECK_IN_GUESTS AS
  v_yesterday DATE := TRUNC(SYSDATE) - 1;
BEGIN
  FOR booking_rec IN (
    SELECT booking_id
    FROM BOOKING
    WHERE TRUNC(booking_start_date) = v_yesterday
  ) LOOP
    BEGIN
      UPDATE BOOKING
      SET booking_state = 3
      ...
      DBMS_OUTPUT.PUT_LINE('Бронь с ID ' || booking_rec.booking_id || ' одобрена администратором.');
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('Бронь с ID ' || booking_rec.booking_id || ' не найдена.');
      WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Произошла ошибка: ' || SQLERRM);
    END;
  END LOOP;
